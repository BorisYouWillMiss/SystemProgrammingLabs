...
void DrawAxes(HWND hwnd, int windowX, int windowY)
{
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        for (i = 0; i < NUM; i++)
        {
            ...
            {
            case 1:
                apt[i].y = ((int)(cyClient / 2 * (1 - sin(TWOPI * (i - NUM / 2) / NUM * scale) / scale)));
                break;
            case 2:
                apt[i].y = (int)(cyClient / 2 * (1 - cos(TWOPI * (i - NUM / 2) / NUM * scale) / scale));
                break;
            case 3:
                apt[i].y = (int)(cyClient / 2 * (1 - tan(TWOPI * (i - NUM / 2) / NUM * scale) / scale));
                break;
            case 4:
                apt[i].y = (int)(cyClient / 2 * (1 - (1/tan((TWOPI * (i - NUM / 2) / NUM * scale))) / scale));
                break;
            }
        }

        DrawAxes(hwnd, cxClient, cyClient);
...
void AddMenus(HWND hwnd)
{
    ...
    AppendMenu(hFunctionMenu, MF_STRING, 4, L"Cot");

    HMENU hScaleMenu = CreateMenu();

    AppendMenu(hScaleMenu, MF_STRING, 5, L"1x");
    AppendMenu(hScaleMenu, MF_STRING, 6, L"2x");
    AppendMenu(hScaleMenu, MF_STRING, 7, L"3x");
    AppendMenu(hScaleMenu, MF_STRING, 8, L"4x");
    AppendMenu(hScaleMenu, MF_STRING, 9, L"5x");
    AppendMenu(hScaleMenu, MF_STRING, 10, L"6x");
